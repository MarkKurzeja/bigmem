...
def _put(db: str, key: str, value: str, namespace: str = "default", tags: str = "") -> dict:
    cmd = [sys.executable, "-m", "bigmem", "--db", db, "--namespace", namespace, "put", key, value]
    if tags:
        cmd += ["--tags", tags]
...
def _get(db: str, key: str, namespace: str = "default") -> dict:
    r = subprocess.run(
        [sys.executable, "-m", "bigmem", "--db", db, "--namespace", namespace, "get", key],
...
def _append(db: str, key: str, value: str, namespace: str = "default") -> dict:
    r = subprocess.run(
        [sys.executable, "-m", "bigmem", "--db", db, "--namespace", namespace, "append", key, value],
        capture_output=True, text=True, timeout=30,
    )
    return {"rc": r.returncode, "stdout": r.stdout.strip(), "stderr": r.stderr.strip()}
...
def _search(db: str, query: str, namespace: str = "default") -> dict:
    r = subprocess.run(
        [sys.executable, "-m", "bigmem", "--db", db, "--namespace", namespace, "search", query],
        capture_output=True, text=True, timeout=30,
    )
    return {"rc": r.returncode, "stdout": r.stdout.strip(), "stderr": r.stderr.strip()}
...
def _list_keys(db: str, namespace: str = "default") -> dict:
    r = subprocess.run(
        [sys.executable, "-m", "bigmem", "--db", db, "--namespace", namespace, "list", "--keys-only"],
        capture_output=True, text=True, timeout=30,
    )
    return {"rc": r.returncode, "keys": json.loads(r.stdout) if r.returncode == 0 else []}
...
class TestConcurrentWrites:
    ...
    def test_parallel_puts_no_conflicts(self, tmp_path):
        ...
        with ProcessPoolExecutor(max_workers=8) as pool:
            futures = {
                pool.submit(_put, db, f"key-{i}", f"value-{i}"): i
                for i in range(n)
            }
            results = {}
    ...
    def test_parallel_puts_same_key_upsert(self, tmp_path):
        ...
        with ProcessPoolExecutor(max_workers=8) as pool:
            futures = [
                pool.submit(_put, db, "contended-key", f"value-{i}")
                for i in range(n)
            ]
...
class TestConcurrentReadsWrites:
    ...
    def test_reads_dont_block_writes(self, tmp_path):
        ...
        with ProcessPoolExecutor(max_workers=8) as pool:
            write_futures = [
                pool.submit(_put, db, f"new-{i}", f"new-value-{i}")
                for i in range(n_writers)
            ]
            read_futures = [
                pool.submit(_get, db, f"seed-{i % 10}")
...
class TestConcurrentNamespaces:
    ...
    def test_namespace_isolation_concurrent(self, tmp_path):
        ...
        with ProcessPoolExecutor(max_workers=8) as pool:
            ...
            for agent in range(n_agents):
                ...
                for i in range(n_facts_per_agent):
                    futures.append(
                        pool.submit(_put, db, f"fact-{i}", f"agent-{agent}-value-{i}", namespace=ns)
                    )
            results = [f.result() for f in futures]
        ...
        for agent in range(n_agents):
            ...
            assert len(keys) == n_facts_per_agent, f"Namespace {ns}: expected {n_facts_per_agent}, got {len(keys)}"
...
class TestConcurrentAppend:
    ...
    def test_concurrent_appends_to_same_key(self, tmp_path):
        ...
        with ProcessPoolExecutor(max_workers=8) as pool:
            futures = [
                pool.submit(_append, db, "log", f"entry-{i}")
                for i in range(n)
            ]
            results = [f.result() for f in futures]
        ...
        assert len(fact["value"]) == n, (
            f"Expected {n} entries but got {len(fact['value'])} — "
            f"possible race condition in append"
        )
...
class TestConcurrentSearch:
    ...
    def test_search_during_writes(self, tmp_path):
        ...
        with ProcessPoolExecutor(max_workers=8) as pool:
            ...
            write_futures = [
                pool.submit(_put, db, f"extra-{i}", f"extra data {i}")
                for i in range(20)
            ]
            # Readers searching
            search_futures = [
                pool.submit(_search, db, "important finding")
                for _ in range(10)
            ]
...
class TestCrossProcessCommunication:
    ...
    def test_producer_consumer_pattern(self, tmp_path):
        ...
        for i in range(n_items):
            _put(db, f"task-{i}", json.dumps({"status": "pending", "data": f"work-{i}"}),
                 namespace="tasks", tags="pending")
