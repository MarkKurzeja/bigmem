...
class TestSubprocessBenchmarks:
    ...
    def test_bench_cli_put_get_cycle(self, tmp_path):
        ...
        def cli_put(i):
            subprocess.run(
                [sys.executable, "-m", "bigmem", "--db", db, "put", f"key-{i}", f"value-{i}", "-q"],
        ...
        # Batch (amortized)
        lines = "\n".join(
            json.dumps({"op": "put", "key": f"batch-{i}", "value": f"v-{i}"})
            for i in range(n)
        )
        start = time.perf_counter()
        r = subprocess.run(
        ...
        print(f"  CLI batch ({n} puts): {batch_elapsed:.3f}s ({batch_elapsed/n*1000:.1f} ms/op amortized)")
    ...
    def test_bench_cli_search(self, tmp_path):
        ...
        # Seed via batch
        lines = "\n".join(
            json.dumps({"op": "put", "key": f"doc-{i}", "value": f"important finding number {i}"})
            for i in range(100)
        )
        subprocess.run(
            [sys.executable, "-m", "bigmem", "--db", db, "batch"],
        ...
        for _ in range(n):
            subprocess.run(
                [sys.executable, "-m", "bigmem", "--db", db, "search", "important finding"],
                capture_output=True, timeout=10,
            )
        elapsed = time.perf_counter() - start
        print()
    ...
    def test_bench_cli_stats(self, tmp_path):
        ...
        # Seed
        lines = "\n".join(
            json.dumps({"op": "put", "key": f"k-{i}", "value": f"v-{i}", "tags": f"tag-{i%5}"})
            for i in range(500)
        )
        subprocess.run(
            [sys.executable, "-m", "bigmem", "--db", db, "batch"],
