...
def _output(data) -> None:
  if isinstance(data, str):
    ...
    sys.stdout.write(
      json.dumps(data, indent=indent, separators=(",", ":") if not _pretty else None) + "\n"
    )
...
def main():
  parser = argparse.ArgumentParser(
    prog="bigmem", description="SQLite-backed memory store for AI agents"
  )
  ...
  p_list.add_argument(
    "--since", default="", help="Only facts created at or after this ISO timestamp"
  )
  ...
  p_search.add_argument(
    "--exact",
    action="store_true",
    help="Disable smart OR-join; use raw FTS5 AND matching",
  )
  ...
  p_cleanup.add_argument(
    "--before", default="", help="Delete facts created before this ISO timestamp"
  )
